...
WhatsApp group echo suppression is intelligent: OpenClaw tracks recent gateway-sent message IDs and only suppresses matching echoes. This means owner commands like `/status`, `/new`, and `/activation` from linked-account `fromMe` traffic are preserved and no longer incorrectly filtered.
