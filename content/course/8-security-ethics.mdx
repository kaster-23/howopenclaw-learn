...
faqs:
  ...
  - q: "How do I protect my AI provider API key?"
    a: "Your key is stored in ~/.openclaw/openclaw.json. Restrict file permissions with `chmod 600 ~/.openclaw/openclaw.json` so only your user account can read it. Never commit this file to git, never share it, and set spending limits on your provider's dashboard so a compromised key can't cause large charges."
...
OpenClaw <OpenClawVersion /> includes several security enhancements:
