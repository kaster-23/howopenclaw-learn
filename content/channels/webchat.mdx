...
WebChat only listens on localhost. It's not accessible from other devices on your network unless you've explicitly configured remote access (see [Network Security](/foundation/security)).
...
**No persistent history.** Conversation history isn't persisted between sessions in WebChat. For persistent memory, see the [Memory System](/level-up/memory) guide.
