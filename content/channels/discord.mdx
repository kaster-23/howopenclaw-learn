...
Discord works well for community-facing use cases and casual personal use. The setup requires creating a bot application in Discord's Developer Portal — slightly more involved than Telegram, but well-documented. Discord runs on any [supported OpenClaw host](/reference/system-requirements), and costs only your [AI provider's API fees](/reference/pricing) — there's no Discord subscription or per-server charge.
...
<Callout type="info" title="Reply routing preserved (v2026.4.23)">
