...
faqs:
  - q: "What is the OpenClaw Gateway?"
    a: "The Gateway is the always-running background process at the heart of [OpenClaw](https://openclaw.ai). It listens on port 18789, manages all channels, handles incoming messages, runs the agent, and executes cron jobs."
  - q: "What are OpenClaw Skills?"
    a: "Skills are extensions that give your agent new capabilities — web search, email, calendar, GitHub, and more. Each skill is a Markdown SKILL.md file that describes a tool set the agent can use. Browse available skills on [ClawHub](https://docs.openclaw.ai/skills)."
  - q: "What are OpenClaw Channels?"
    a: "Channels are the messaging platforms your agent communicates through. OpenClaw supports [24+ channels](/channels) including WhatsApp, Telegram, Slack, Discord, iMessage, Signal, and more."
  - q: "What is SOUL.md in OpenClaw?"
    a: "[SOUL.md](https://docs.openclaw.ai/memory/soul) is a Markdown file where you write your agent's personality, communication style, values, and behavioral rules. The agent reads it on every startup to establish its identity."
  - q: "What are Cron Jobs in OpenClaw?"
    a: "Cron jobs are scheduled tasks your agent runs automatically on a timer — without you initiating them. Examples include morning briefings, daily summaries, or periodic data fetches. Use [crontab.guru](https://crontab.guru) to build and test cron expressions."
---


Quick reference. Terms are listed in the order you'll encounter them. For a full explanation of what OpenClaw is and how it compares to alternatives, see [What is OpenClaw?](/reference/what-is-openclaw).
...
*In practice: If you set `enforceOwnerForCommands: true`, make sure `commands.ownerAllowFrom` explicitly lists your identity on each channel (e.g., `"telegram:123456789"`). See the [troubleshooting guide](/reference/troubleshooting#authcommands-owner-enforcement) if commands stop responding after upgrading to v2026.4.21.*
