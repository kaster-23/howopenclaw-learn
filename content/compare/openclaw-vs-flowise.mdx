...
faqs:
  - q: "Is OpenClaw built on Flowise?"
    a: "No. OpenClaw and [Flowise](https://flowiseai.com) are independent tools. Both are self-hosted Node.js applications that use LLMs, but they're built separately. Flowise is built on [LangChain](https://www.langchain.com); OpenClaw has its own architecture."
  - q: "Can OpenClaw run local AI models like Ollama?"
    a: "Not currently. OpenClaw supports hosted providers: Anthropic (Claude), OpenAI (GPT-4), and Google (Gemini). [Flowise](https://flowiseai.com) supports 50+ LLM integrations including [Ollama](https://ollama.com) and llama.cpp for local inference, making it more flexible if local model execution is a requirement."
  - q: "Is OpenClaw or Flowise faster to set up for personal use?"
    a: "OpenClaw. It's configured and running in 15 minutes with a one-line installer and a few config fields. [Flowise](https://flowiseai.com) starts quickly but requires designing flows in its visual editor before you get a working assistant — which adds time for anything beyond a basic chatbot."
---
