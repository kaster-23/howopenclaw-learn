...
## Graph Pagination (<OpenClawVersion />)

As of <OpenClawVersion />, Microsoft Graph API pagination is fully supported. When the bot fetches large result sets (like message histories or user lists), results are automatically paginated so you don't hit API limits.

## Message Actions (<OpenClawVersion />)

As of <OpenClawVersion />, your agent can use Teams message actions: pin, unpin, read, react to a message, and list reactions. These are available as tools your agent can invoke during a conversation.
...
<Callout type="warn" title="Security fix in v2026.4.23">
