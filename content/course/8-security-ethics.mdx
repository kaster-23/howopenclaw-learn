...
After configuration, test that restricted commands are blocked:

```bash
# Try to run a forbidden command — it should fail
openclaw agent --message "Run rm -rf /"

# Try to access a blocked path — it should fail
openclaw agent --message "Read from /etc/passwd"

# Check that untrusted content can't override instructions
