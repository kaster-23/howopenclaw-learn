...
faqs:
  ...
  - q: "Does OpenClaw use ChatGPT's AI models?"
    a: "OpenClaw can use [OpenAI's GPT-4 models](https://platform.openai.com/docs/models), yes — but it's not exclusive to them. You choose your AI provider in config: [Anthropic (Claude)](https://www.anthropic.com), OpenAI (GPT-4), or [Google (Gemini)](https://deepmind.google/technologies/gemini/). The difference is where everything runs and how it's accessed, not which model powers responses."
...
If that tradeoff makes sense for you, the [installation guide](/course/0-setup) covers setup. Then pick the messaging channel that fits your life: [Telegram](/channels/telegram) for the fastest setup, [WhatsApp](/channels/whatsapp) if that's where you already communicate, or [Slack](/channels/slack) for a work context.
