...
This opens `~/.openclaw/openclaw.json` in your default text editor (usually VS Code, nano, or Notepad). Find the spot to add channels and add the Telegram section:
...
<Callout type="warn" title="Add to existing config — don't replace it">
Your file already has other settings from the onboarding wizard (your API key, model choice, etc.). Add the `channels` block alongside the existing settings. Do not delete what is already there.
</Callout>

<Callout type="info" title="If openclaw config edit doesn't open an editor">
Run `openclaw config path` to find the config file location, then open it manually in any text editor (Notepad, TextEdit, VS Code).
</Callout>
