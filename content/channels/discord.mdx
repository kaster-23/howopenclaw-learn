...
OpenClaw now handles Discord gateway failures robustly:

- **Centralized error handling** — Early, active, and late-teardown errors are classified consistently and no longer surface as process-killing crashes
- **Stale socket draining** — Poisoned resume state is cleared before forced fresh reconnects, preventing infinite reconnection loops
- **Clean shutdown** — Expected reconnect-exhausted events during intentional lifecycle stop are treated as clean shutdowns instead of failures

Enable reconnection logic in your config for automatic recovery:
