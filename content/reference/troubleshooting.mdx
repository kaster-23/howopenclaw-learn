...
<Callout type="info" title="Channel isolation (v2026.4.5)">
...
### Gateway restart issues (Windows & macOS, v2026.4.5)
...
5. As of v2026.4.5, voice notes should transcribe automatically — if you see `<media:audio>` placeholders, check logs for transcription errors
...
As of v2026.4.5, watchdog timeouts are reset after reconnect, so quiet chats should no longer fall into reconnect loops.
...
As of v2026.4.5, live DM replies now route correctly to the concrete inbound DM channel, so assistant replies should no longer disappear when pairing.
...
### Teams media and threading (v2026.4.5)
...
As of v2026.4.5, interrupted recurring jobs are replayed on the first gateway restart (not the second), and failure notifications are sent through the job's primary delivery channel using the same session context as successful delivery when no explicit `failureDestination` is configured.
...
As of v2026.4.5, new built-in tools include `video_generate` and `music_generate` for media creation. Check that you've configured the appropriate providers if you want to use these.
...
### Legacy config aliases (v2026.4.5)

As of v2026.4.5, legacy public config aliases like `talk.voiceId`, `talk.apiKey`, and channel/group/room `allow` toggles have been removed in favor of canonical public paths and `enabled`. However, existing configs with legacy aliases still load at startup.
...
### Update with mixed package managers (v2026.4.5)
...
## Memory & Dreaming issues (v2026.4.5)
