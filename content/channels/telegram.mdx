...
If you use Telegram groups with **Topics** enabled (forum-style groups), OpenClaw routes messages to the correct topic automatically. Native commands like `/new` and `/reset` stay routed to the active topic, and the `#General` topic (topic ID `1`) is handled reliably even when Telegram omits forum metadata.
