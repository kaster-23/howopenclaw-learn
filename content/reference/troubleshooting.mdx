...
The network fetch guard now correctly handles trusted environment proxy mode. If you're using a trusted proxy for outbound connections, DNS pinning is automatically disabled so the proxy can resolve outbound hostbound hostnames. This ensures proxy-only sandbox deployments can reach external hosts without manual workarounds.
