...
openclaw config schema              # Print the generated JSON schema for openclaw.json
...
<Callout type="info" title="Update preflight checks">
...
<Callout type="info" title="Config schema inspection (v2026.3.28)">
`openclaw config schema` prints the generated JSON schema for `openclaw.json`. Useful for understanding valid config structures and available options.
</Callout>
...
This works with both Docker and Podman. The CLI detects the running OpenClaw container automatically. As of v2026.3.28, the Podman setup is simplified around the current rootless user, with the launch helper installed under `~/.local/bin`, and the host-CLI `openclaw --container <name>` workflow is the standard way to interact with containerized instances.
