...
**Slack** interactive replies now have full rich reply parity for direct deliveries. Simple trailing `Options:` lines auto-render as buttons or select menus. Interactive setup defaults have been improved and reply controls are isolated from plugin interactive handlers.
...
**Telegram** now handles `#General` forum topic routing reliably (including native commands, interactive callbacks, and fallback error replies), preflights photo dimensions (falling back to document sends when invalid), and stops retrying permanently failed chats (membership/block/kick).
