"use client"

import { useState, useEffect } from "react"
import Link from "next/link"
import { CHALLENGES, TOTAL_XP, STORAGE_KEY, calcXP, getCompleted } from "@/lib/challenge/challenges"

export function ChallengeProgress() {
  const [completed, setCompleted] = useState<string[]>(getCompleted)

  useEffect(() => {
    const onStorage = (e: StorageEvent) => {
      if (e.key === STORAGE_KEY) setCompleted(getCompleted())
    }
    window.addEventListener("storage", onStorage)
    return () => window.removeEventListener("storage", onStorage)
  }, [])

  const xp = calcXP(completed)
  const pct = Math.round((xp / TOTAL_XP) * 100)
  const nextChallenge = CHALLENGES.find((c) => !completed.includes(c.id))
  const allDone = completed.length === CHALLENGES.length

  return (
    <div
      className="not-prose my-6 rounded-xl overflow-hidden"
      style={{ background: "#0f172a", border: "1px solid #1e293b" }}
    >
      {/* Header */}
      <div
        className="px-5 py-3 flex items-center justify-between"
        style={{ background: "#1e293b", borderBottom: "1px solid #334155" }}
      >
        <span style={{ color: "#94a3b8", fontSize: 11, fontFamily: "monospace", letterSpacing: "0.12em" }}>
          CHALLENGE PROGRESS
        </span>
        <span style={{ color: "#64748b", fontSize: 11, fontFamily: "monospace" }}>
          {completed.length} / {CHALLENGES.length} COMPLETE
        </span>
      </div>

      <div className="p-5">
        {/* XP row */}
        <div className="flex items-center justify-between mb-4">
          <div>
            <p style={{ color: "#64748b", fontSize: 11, fontFamily: "monospace", letterSpacing: "0.1em", marginBottom: 2 }}>
              CHALLENGE MODE
            </p>
            <p style={{ color: "#f8fafc", fontSize: 20, fontWeight: 800 }}>30-Minute Run</p>
          </div>
          <div className="text-right">
            <p style={{ color: "#64748b", fontSize: 11, fontFamily: "monospace", letterSpacing: "0.1em", marginBottom: 2 }}>
              TOTAL XP
            </p>
            <p style={{ color: "#f8fafc", fontSize: 20, fontWeight: 800 }}>
              {xp.toLocaleString()}
              <span style={{ color: "#475569", fontSize: 14, fontWeight: 400 }}> / {TOTAL_XP.toLocaleString()}</span>
            </p>
          </div>
        </div>

        {/* Progress bar */}
        <div className="mb-5">
          <div
            className="rounded-full overflow-hidden"
            style={{ height: 10, background: "#1e293b", border: "1px solid #334155" }}
          >
            <div
              className="h-full rounded-full"
              style={{
                width: `${pct}%`,
                background: xp === 0
                  ? "transparent"
                  : "linear-gradient(90deg, #f59e0b, #f97316, #ef4444)",
                boxShadow: xp > 0 ? "0 0 10px #f97316aa" : "none",
                transition: "width 1s ease",
                minWidth: xp > 0 ? 8 : 0,
              }}
            />
          </div>
        </div>

        {/* Challenge dots */}
        <div className="flex gap-2 mb-5">
          {CHALLENGES.map((c) => {
            const done = completed.includes(c.id)
            const isNext = !done && completed.length === c.num - 1
            return (
              <Link
                key={c.id}
                href={c.href}
                title={`Challenge ${c.num}: ${c.label}`}
                className="flex-1 rounded flex flex-col items-center gap-1 py-2 transition-all"
                style={{
                  background: done ? "#78350f22" : isNext ? "#1e293b" : "#0f172a",
                  border: done
                    ? "1px solid #f59e0b44"
                    : isNext
                    ? "1px solid #f59e0b44"
                    : "1px solid #1e293b",
                  textDecoration: "none",
                }}
              >
                <span style={{ fontSize: 14 }}>
                  {done ? "✓" : isNext ? "▶" : "·"}
                </span>
                <span
                  style={{
                    fontSize: 9,
                    fontFamily: "monospace",
                    color: done ? "#fbbf24" : isNext ? "#fbbf24" : "#475569",
                    letterSpacing: "0.05em",
                  }}
                >
                  C{c.num}
                </span>
              </Link>
            )
          })}
        </div>

        {/* CTA */}
        {allDone ? (
          <Link
            href="/challenge/complete"
            className="flex items-center justify-between rounded-lg px-4 py-3"
            style={{
              background: "linear-gradient(135deg, #78350f, #92400e)",
              border: "1px solid #f59e0b44",
              color: "#fbbf24",
              fontWeight: 700,
              fontSize: 14,
              textDecoration: "none",
            }}
          >
            <span>🏆 Claim Your Certificate</span>
            <span>→</span>
          </Link>
        ) : nextChallenge ? (
          <Link
            href={nextChallenge.href}
            className="flex items-center justify-between rounded-lg px-4 py-3"
            style={{
              background: "#1e293b",
              border: "1px solid #334155",
              color: "#e2e8f0",
              fontWeight: 600,
              fontSize: 14,
              textDecoration: "none",
            }}
          >
            <span>
              {completed.length === 0
                ? "Start Challenge 1: Install"
                : `Continue: Challenge ${nextChallenge.num} — ${nextChallenge.label}`}
            </span>
            <span style={{ color: "#64748b" }}>→</span>
          </Link>
        ) : null}
      </div>
    </div>
  )
}
